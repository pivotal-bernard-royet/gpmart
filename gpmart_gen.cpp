...
static void yearUsage() {
    printf("Please type \"--years startYear-endYear\".\n");
}
...
static void getYear(char *optarg) {
    ...
    *p = '\0';
    ...
    if (!allDigits(optarg) || startYear == 0) {
        printf("%s contains non-digit characters.\n", optarg);
        exit(1);
    }
    else if (!allDigits(p+1) || endYear == 0) {
        printf("%s contains non-digit characters.\n", p+1);
        exit(1);
    }
...
main(int argc, char **argv)
{
 ...
 if (!any) printf("Using default parameters\n");
...
/* Print any remaining command line arguments (not options). */
 if (optind < argc)
 {
  ...
  printf("null = '%s'\n", nullflag);
  printf("timefile = %s\n", timefile);
  printf("numfactsalesrows = %lld\n", numfactsalesrows);
  printf("numfactorderrows = %lld\n", numfactorderrows);
  printf("numprodkeys = %lld\n", numprodkeys);
  printf("numstorekeys = %lld\n", numstorekeys);
  printf("numpromokeys = %lld\n", numpromokeys);
  printf("numvendkeys = %lld\n", numvendkeys);
  printf("numcustkeys = %lld\n", numcustkeys);
  printf("numempkeys = %lld\n", numempkeys);
  printf("numwarehousekeys = %lld\n", numwarehousekeys);
  printf("numshippingkeys = %lld\n", numshippingkeys);
  printf("numonlinepagekeys = %lld\n", numonlinepagekeys);
  printf("numcallcenterkeys = %lld\n", numcallcenterkeys);
  printf("numfactonlinesalesrows = %lld\n", numfactonlinesalesrows);
  printf("numinventoryfactrows = %lld\n", numfactinventoryrows);
  
  sprintf(path,"mkdir -p %s",datadirectory);
  datadirstatus = system(path);
  if(datadirstatus != 0)
  {
    printf("Please provide different data directory path.\n");
    exit(1);
  }
...
void store_gen(const char *file, uint64 numstores)
{
  ...
  for (uint64 i = 1; i <= numstores; i++)
  {
    ...
    fprintf(fd,"|%d", (int)(sqfoot * flip(1, 5) * 0.1));
...
#define NUM_FOODS 7
  ...
  if (fd == NULL)
  {
      printf("Error : Can't create Product_Dimension output file !!\n");
      exit(1);
  }
...
void promo_gen(const char *file, uint64 numpromos)
{
  ...
  if (fd == NULL)
  {
      printf("Error : Can't create Promotion_Dimension output file !!\n");
      exit(1);
  }
...
void vend_gen(const char *file, uint64 numvends)
{
  ...
  if (fd == NULL)
  {
      printf("Error : Can't create Vendor_Dimension output file !!\n");
      exit(1);
  }
  ...
  for(uint64 i = 1; i <= numvends; i++)
  {
    ...
    fprintf(fd,"%lld|%s %s|%lld %s|%s|%s|%s|%lld|%lld-%lld-%lld\n",
             ...
             flip(1, 12), flip(1, 28), flip(startYear, endYear)    // last deal update
            );
  }
  fclose(fd);
}
...
void cust_gen(const char *file, uint64 numcusts)
{
  ...
  if (fd == NULL)
  {
      printf("Error : Can't create Customer_Dimension output file !!\n");
      exit(1);
  }
  ...
  for(uint64 i = 1; i <= numcusts; i++)
  {
    ...
    else    // customer is a company
    {
      ...
      fprintf(fd,"|%lld-%lld-%lld|%s|%lld|%lld-%lld-%lld",
              ...
              flip(1, 28), flip(startYear, endYear)
             );
    }
    fprintf(fd, "\n");
  }
...
void emp_gen(const char *file, uint64 numemps)
{
  ...
  if (fd == NULL)
  {
      printf("Error : Can't create Employee_Dimension output file !!\n");
      exit(1);
  }
...
uint64 time_gen(const char *timefile, const char *outfile)
{
  ...
  while (getline(&line, &linesize, fd) != -1)
  {
    ...
    if (checkStartYear) {
        checkStartYear = 0;
        // print info for out of range
        if (startYear == year-1)
            printf("Time.txt does not contain data for year of %d.\n", startYear);
        else if (startYear < year)
            printf("Time.txt does not contain data for years from %d to %lld.\n", startYear, year-1);
    }
    ...
    fprintf(ofd,"|%d",(weekDay == 7) ? 1:0); // last day of week
    fprintf(ofd,"|%d",
    ...
    fprintf(ofd,"|%d", (monthYear < 4 ? 1:
                   monthYear < 7 ? 2:
                   monthYear < 10 ? 3: 4));
    fprintf(ofd,"|%lld-%s", year, quarter[monthYear]);
    fprintf(ofd,"|%d", monthYear <=6 ? 1:2);
  ...
  if (endYear == year+1)
      ...
      printf("Time.txt does not contain data for years from %lld to %d.\n", year+1, endYear);
...
void warehouse_gen(const char *file, uint64 numwarehouses)
{
  ...
  if (fd == NULL)
  {
      printf("Error : Can't create Warehouse_Dimension output file !!\n");
      exit(1);
  }
...
void shipping_gen(const char *file, uint64 numshipping)
{
  ...
  if (fd == NULL)
  {
      printf("Error : Can't create Shipping_Dimension output file !!\n");
      exit(1);
  }
...
void onlinepage_gen(const char *file, uint64 numonlinepages)
{
  ...
  if (fd == NULL)
  {
      printf("Error : Can't create Onlinepage_Dimension output file !!\n");
      exit(1);
  }
...
void callcenter_gen(const char *file, uint64 numcallcenters)
{
  ...
  if (fd == NULL)
  {
      printf("Error : Can't create Callcenter_Dimension output file !!\n");
      exit(1);
  }
...
void make_loadnull()
{
  ...
  for(int i = 0; i < len; i++)
  {
    ...
    if(nullflag[i] == '\\')
    {
      ...
      if(i == len-1)
      {
        printf("The null value cannot end in a '\\'\n");
        exit(1);
      }
    }
  }
...
void gen_load_script(const char *filename,uint64 numfiles,int pwd)
{
  ...
  if(fd == NULL)
  {
    printf("Cannot create data load file.\n");
    exit(1);
  }
